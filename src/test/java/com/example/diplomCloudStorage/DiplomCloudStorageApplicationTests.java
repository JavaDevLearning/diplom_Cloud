package com.example.diplomCloudStorage;
...
import com.example.diplomCloudStorage.dto.request.FileNameInRequest;
...
import com.example.diplomCloudStorage.repository.FileRepository;
...
class DiplomCloudStorageApplicationTests {
    ...
    public void testLogin() {
        userRepository.save(new UserEntity("KarevaElena", "qweasdzxc"));
        final HttpHeaders headers = new HttpHeaders();
        headers.set("Content-Type", "application/json");

        final LoginInRequest operation = new LoginInRequest("KarevaElena", "qweasdzxc");
        final HttpEntity<LoginInRequest> request = new HttpEntity<>(operation, headers);

        final ResponseEntity<LoginResponseDto> result = this.restTemplate.postForEntity("/login", request, LoginResponseDto.class);
    ...
    public void testLogout() {
        final String authToken = "Bearer 555";
